"""Comprehensive demo of agentdocker-lite API.
...
# Timeout
_, ec = sb.run("sleep 60", timeout=1)
print(f"  timeout exit_code={ec} (124=timeout)")
...
sb.fs_snapshot("/tmp/adl_snapshot_test")
print("  snapshot saved")

sb.run("echo modified > /workspace/data.txt")
sb.fs_restore("/tmp/adl_snapshot_test")
...
_shutil.rmtree("/tmp/adl_snapshot_test")
sb.delete()
print("  OK")
...
sb.save_as_image("adl-dev-test:cached")
print("  saved as Docker image: adl-dev-test:cached")

sb.delete()

# Verify: create from cached image
sb2 = Sandbox(SandboxConfig(image="adl-dev-test:cached", working_dir="/workspace"), name="from-cache")
output, ec = sb2.run("cat /workspace/cached.txt")
print(f"  from cached image: {output.strip()!r}, ec={ec}")
sb2.delete()

import subprocess as _sp
_sp.run(["docker", "rmi", "-f", "adl-dev-test:cached"], capture_output=True)
print("  OK")
...
cleaned = SandboxBase.cleanup_stale()
...
if _os.path.exists("/dev/fuse"):
    ...
    print(f"  features['devices']: {sb.features['devices']}")
...
if QemuVM.check_available():
    vm_dir = Path(tempfile.mkdtemp(prefix="adl_vm_"))
    ...
    # Check if QEMU is available in sandbox
...
_compose_dir = _tempfile.mkdtemp(prefix="adl_compose_")
...
_compose_dir2 = _tempfile.mkdtemp(prefix="adl_compose2_")
