...
fn main() {
    ...
    match subcommand.as_str() {
        ...
        | "snapshot-mounts" | "snapshot-remove" => {}
        "--help" | "-h" | "help" => usage(),
        _ => die(&format!("unknown subcommand: {subcommand}")),
    }

    // ---- snapshot-mounts subcommand --------------------------------------
    // Resolve containerd snapshot layer paths for a Docker image.
    // Calls ctr (as root) to create a read-only snapshot view and returns
    // the overlayfs lowerdir paths (one per line on stdout).
    if subcommand == "snapshot-mounts" {
        let mut image: Option<&str> = None;
        let mut view_key: Option<&str> = None;
        let mut j = 0;
        while j < criu_args.len() {
            match criu_args[j].as_str() {
                "--image" if j + 1 < criu_args.len() => { image = Some(&criu_args[j+1]); j += 2; }
                "--view-key" if j + 1 < criu_args.len() => { view_key = Some(&criu_args[j+1]); j += 2; }
                _ => j += 1,
            }
        }
        let image = image.unwrap_or_else(|| die("snapshot-mounts: --image required"));
        let view_key = view_key.unwrap_or_else(|| die("snapshot-mounts: --view-key required"));

        // Validate view-key (alphanumeric + hyphen only)
        if !view_key.chars().all(|c| c.is_alphanumeric() || c == '-') {
            die("snapshot-mounts: --view-key must be alphanumeric/hyphen only");
        }

        // 1. Get diff_ids from docker inspect
        let output = Command::new("docker")
            .args(["inspect", image, "--format", "{{json .RootFS.Layers}}"])
            .output()
            .unwrap_or_else(|e| die(&format!("docker inspect: {e}")));
        if !output.status.success() {
            die(&format!("docker inspect failed for {image}"));
        }
        let layers_json: Vec<String> = serde_json::from_slice(&output.stdout)
            .unwrap_or_else(|e| die(&format!("parse diff_ids: {e}")));

        // 2. Compute chain IDs
        use sha2::{Sha256, Digest};
        let mut prev = String::new();
        let mut top_chain = String::new();
        for did in &layers_json {
            if prev.is_empty() {
                top_chain = did.clone();
            } else {
                let input = format!("{prev} {did}");
                let hash = Sha256::digest(input.as_bytes());
                top_chain = format!("sha256:{:x}", hash);
            }
            prev = top_chain.clone();
        }

        // 3. Create read-only snapshot view
        let out = Command::new("ctr")
            .args(["--namespace", "moby", "snapshot", "view", view_key, &top_chain])
            .output()
            .unwrap_or_else(|e| die(&format!("ctr snapshot view: {e}")));
        if !out.status.success() {
            die(&format!("ctr snapshot view failed: {}", String::from_utf8_lossy(&out.stderr)));
        }

        // 4. Get mount spec
        let out = Command::new("ctr")
            .args(["--namespace", "moby", "snapshot", "mounts", "/tmp/_unused", view_key])
            .output()
            .unwrap_or_else(|e| die(&format!("ctr snapshot mounts: {e}")));
        if !out.status.success() {
            // Cleanup view on failure
            let _ = Command::new("ctr")
                .args(["--namespace", "moby", "snapshot", "rm", view_key])
                .output();
            die(&format!("ctr snapshot mounts failed: {}", String::from_utf8_lossy(&out.stderr)));
        }

        // 5. Parse lowerdir paths from mount output
        // Format: "mount -t overlay overlay /tmp/x -o lowerdir=...:...,index=off\n"
        let mount_spec = String::from_utf8_lossy(&out.stdout);
        let lowerdir = mount_spec
            .find("lowerdir=")
            .map(|start| {
                let rest = &mount_spec[start + "lowerdir=".len()..];
                // lowerdir value ends at the next comma or whitespace/newline
        ...
        match lowerdir {
            ...
            _ => {
                let _ = Command::new("ctr")
                    .args(["--namespace", "moby", "snapshot", "rm", view_key])
                    .output();
                die("no lowerdir found in ctr snapshot mounts output");
            }
        }
        exit(0);
    }
    ...
    if subcommand == "snapshot-remove" {
        let view_key = criu_args.first().unwrap_or_else(|| die("snapshot-remove: VIEW_KEY required"));
        if !view_key.chars().all(|c| c.is_alphanumeric() || c == '-') {
            die("snapshot-remove: view key must be alphanumeric/hyphen only");
        }
        let out = Command::new("ctr")
            .args(["--namespace", "moby", "snapshot", "rm", view_key])
            .output()
            .unwrap_or_else(|e| die(&format!("ctr snapshot rm: {e}")));
        if !out.status.success() {
            die(&format!("ctr snapshot rm failed: {}", String::from_utf8_lossy(&out.stderr)));
        }
        exit(0);
    }
